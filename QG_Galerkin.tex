...
\pd{t}\vartheta^-+ \tJ[\psi^-,\vartheta^-] = 0\\
\nabla_h^2\psi + \pd{z}\left(\frac{f_0^2}{H^2N^2(z)}\pd{z}\psi\right) = q,\quad S(z) = \frac{f_0^2}{H^2N^2(z)}
\end{gather}
where 
\begin{equation}
\vartheta^\pm= \frac{f_0^2}{H^2N^2}\pd{z}\psi\text{ at }z=0,1
\end{equation}
...
q_\sN^G-\left(\nabla_h^2\psi_\sN^G + \pd{z}\left(\frac{f_0^2}{H^2N^2(z)}\pd{z}\psi_\sN^G\right)\right)=r_q\neq0\\
\vartheta^++r_\vartheta^+= (f_0/(HN))^2\pd{z}\psi_\sN^G\text{ at }z=1,\text{ and }\vartheta^-+r_\vartheta^- = (f_0/(HN))^2\pd{z}\psi_\sN^G\text{ at }z=0.
\end{gather}
...
\[\int-\psi_\sN^G\pd{t}\left(\nabla_h^2\psi_\sN^G + \pd{z}\left(\frac{f_0^2}{H^2N^2(z)}\pd{z}\psi_\sN^G\right) + r_q\right) = \int\psi_\sN^G(\tJ[\psi_\sN^G,q_\sN^G] - r_{qt}) = -\int\psi_\sN^Gr_{qt}.\]
...
Next
\begin{align}
\int-\psi_\sN^G\pd{z}\left(\frac{f_0^2}{H^2N^2(z)}\pd{zt}\psi_\sN^G\right) &= -\int_x\left[\frac{f_0^2}{H^2N^2}\psi_\sN^G\pd{zt}\psi_\sN^G\right]_-^+ + \frac{1}{2}\dd{}{t}{}\int\left(\frac{f}{HN}\pd{z}\psi_\sN^G\right)^2\\
&=  -\int_x\left[\psi_\sN^G(\pd{t}\vartheta+\pd{t}r_\vartheta)\right]_-^+ + \frac{1}{2}\dd{}{t}{}\int\left(\frac{f_0}{HN}\pd{z}\psi_\sN^G\right)^2\\
&=  -\int_x\left[\psi_\sN^G(-\tJ[\psi_\sN^G,\vartheta]+\pd{t}r_\vartheta)\right]_-^+ + \frac{1}{2}\dd{}{t}{}\int\left(\frac{f_0}{HN}\pd{z}\psi_\sN^G\right)^2\\
&=-\int_x\left[\psi_\sN^G\pd{t}r_\vartheta\right]_-^++ \frac{1}{2}\dd{}{t}{}\int\left(\frac{f_0}{HN}\pd{z}\psi_\sN^G\right)^2.
\end{align}
Putting it all together,
\[\frac{1}{2}\dd{}{t}{}\int |\nabla_h\psi_\sN^G|^2 + \left(\frac{f}{HN}\pd{z}\psi_\sN^G\right)^2 =\int_x\left[\psi_\sN^G\pd{t}r_\vartheta\right]_-^++ \int\psi_\sN^G\pd{t}r_q-\int\psi_\sN^Gr_{qt}.\]
...
\begin{equation}
q -\vartheta^+\delta(z-1) + \vartheta^-\delta(z) = \nabla_h^2\psi + \pd{z}\left(\frac{f_0^2}{H^2N^2}\pd{z}\psi\right)
\end{equation}
...
\begin{equation}
q_\sN^G -\vartheta^+\delta(z-1) + \vartheta^-\delta(z) = \nabla_h^2\psi_\sN^G + \pd{z}\left(\frac{f_0^2}{H^2N^2}\pd{z}\psi_\sN^G\right) + r_q^B.
\end{equation}
...
\[\int -\psi_\sN^G\pd{t}q_\sN^G = \int -\psi_\sN^G\pd{t}\left(\nabla_h^2\psi_\sN^G + \pd{z}\left(\frac{f_0^2}{H^2N^2(z)}\pd{z}\psi_\sN^G\right)+\vartheta^+\delta(z-1)-\vartheta^-\delta(z)+r_q^B\right).\]
Again, 
\[\int -\psi_\sN^G\pd{t}\nabla_h^2\psi_\sN^G = \frac{1}{2}\dd{}{t}{}\int|\nabla_h\psi_\sN^G|^2.\]
As before
\begin{align}
\int-\psi_\sN^G\pd{z}\left(\frac{f_0^2}{H^2N^2(z)}\pd{zt}\psi_\sN^G\right) &= -\int_x\left[\frac{f_0^2}{H^2N^2}\psi_\sN^G\pd{zt}\psi_\sN^G\right]_0^1 + \frac{1}{2}\dd{}{t}{}\int\left(\frac{f}{HN}\pd{z}\psi_\sN^G\right)^2=\frac{1}{2}\dd{}{t}{}\int\left(\frac{f_0}{HN}\pd{z}\psi_\sN^G\right)^2.
\end{align}
...
This leaves
\[\int -\psi_\sN^G\pd{t}q_\sN^G= \frac{1}{2}\dd{}{t}{}\int |\nabla_h\psi_\sN^G|^2 + \left(\frac{f}{HN}\pd{z}\psi_\sN^G\right)^2 - \int\psi_\sN^G\pd{t}r_q^B.\]
The energy equation is now
\[\frac{1}{2}\dd{}{t}{}\int |\nabla_h\psi_\sN^G|^2 + \left(\frac{f}{HN}\pd{z}\psi_\sN^G\right)^2 = \int\psi_\sN^G\pd{t}r_q^B-\int\psi_\sN^Gr_{qt}.\]
...
\[-\int(\psi-\psi_\sN^G)\left[\nabla^2(\psi-\psi_\sN^G) + \pd{z}\left(\frac{f_0^2}{H^2N^2}\pd{z}(\psi-\psi_\sN^G)\right)\right]\]
...
\[\mat{B}_{ij} = \int p_i^\psi(z)p_j^q(z)\text{d}z,\;\;\mat{M}_{ij}=\int p_i^\psi(z)p_j^\psi(z)\text{d}z,\;\;\mat{L}_{ij} = \int \frac{f_0^2}{H^2N^2}(\pd{z}p_i^\psi(z))(\pd{z}p_j^\psi(z))\text{d}z\]
...
\hrule
\begin{center}{\bf Linear instability problem}\end{center}
The following is an exact solution of the fully-nonlinear QG equations: 
\[\bar\psi = -\bar u(z)y,\;\;\bar q = -y\dd{}{z}{}\left(\frac{f_0^2}{H^2N^2(z)}\dd{\bar u}{z}{}\right),\;\;\bar\vartheta^\pm = -y\frac{f_0^2}{H^2N^2(z)}\dd{\bar u^\pm}{z}{}.\]
...
\begin{gather}
\pd{t}\vartheta^+ + \bar u^+\pd{x}\vartheta^+-\frac{f_0^2}{H^2N^2}(\pd{x}\psi^+)\dd{\bar u^+}{z}{} = 0\\
\pd{t}q+\bar u(z) \pd{x}q + (\pd{x}\psi)\pd{y}\bar q+\beta (\pd{x}\psi) = 0\\
\pd{t}\vartheta^- + \bar u^-\pd{x}\vartheta^--\frac{f_0^2}{H^2N^2}(\pd{x}\psi^-)\dd{\bar u^-}{z}{} = 0\\
\nabla^2\psi + \dd{}{z}{}\left(\frac{f_0^2}{H^2N^2(z)}\dd{\psi}{z}{}\right) = q-\vartheta^+\delta(z-1)+\vartheta^-\delta(z)
\end{gather}
Coefficients don't vary in the horizontal, so we take the Fourier transform
\begin{gather}
\pd{t}\hat{\vartheta}^+ + \imag k_x\bar u^+\hat{\vartheta}^+-\imag k_x\frac{f_0^2}{H^2N^2}\dd{\bar u^+}{z}{}\hat{\psi}^+ = 0\\
\pd{t}\hat{q}+\imag k_x\bar u(z) \hat{q} + \imag k_x(\pd{y}\bar{q})\hat{\psi}+\imag k_x\beta \hat{\psi} = 0\\
\pd{t}\hat{\vartheta}^- + \imag k_x\bar u^-\hat{\vartheta}^--\imag k_x\dd{\bar u^-}{z}{}\frac{f_0^2}{H^2N^2}\hat{\psi}^- = 0\\
-(k_x^2+k_y^2)\hat{\psi} + \dd{}{z}{}\left(\frac{f_0^2}{H^2N^2(z)}\dd{\hat{\psi}}{z}{}\right) = \hat{q} -\hat{\vartheta}^+\delta(z-1)+\hat{\vartheta}^-\delta(z).
\end{gather}
We look for exponential growth so we try to find solutions with $\pd{t}\hat{q} = -\imag k_x c\hat{q}$
\begin{gather}
-c\hat{\vartheta}^+ + \bar u^+\hat{\vartheta}^+-\frac{f_0^2}{H^2N^2}\dd{\bar u^+}{z}{}\hat{\psi}^+ = 0\\
-c\hat{q}+\bar u(z) \hat{q} +(\pd{y}\bar q)\hat{\psi}+\beta \hat{\psi} = 0\\
-c\hat{\vartheta}^- + \bar u^-\hat{\vartheta}^--\dd{\bar u^-}{z}{}\frac{f_0^2}{H^2N^2}\hat{\psi}^- = 0\\
-(k_x^2+k_y^2)\hat{\psi} + \dd{}{z}{}\left(\frac{f_0^2}{H^2N^2(z)}\dd{\hat{\psi}}{z}{}\right) = \hat{q} -\hat{\vartheta}^+\delta(z-1)+\hat{\vartheta}^-\delta(z).
\end{gather}
...
First, set
\[\bar u_\sN^G = \sum_{n=1}^\sN \bar u_n p_n^\psi(z).\]
Given a $\bar u(z)$ we need to find $\bar u_n$. 
We can choose these coefficients to minimize the $L^2$ norm of the error $\bar u(z) - \bar u_\sN^G(z)$, which we accomplish via a standard Galerkin condition.
We need to solve the system of equations
\begin{equation}
\sum_{k=1}^\sN \int_0^1 p_n^\psi(z)p_k^\psi(z)\bar u_k\text{d}z = \int_0^1 p_n^\psi(z)\bar u(z)\text{d}z ,\;\;n = 1,\ldots,\sN.
\end{equation}
This is a system of the form $\mat{M}\bm{u} = \bm{b}$, and we already know how to compute {\bf M}.
We just need a routine to compute the integrals on the RHS, e.g. using Gauss-Legendre quadrature.

Next we need a Galerkin approximation for $\pd{y}\bar{q}$.
Our standard approach tells us that
\begin{equation}
\mat{B}\pd{y}\bar{\bm{q}} = \mat{L}\pd{y}\bar{\bm{\psi}}+\pd{y}\bar\vartheta^+\bm{p}^+-\pd{y}\bar\vartheta^-\bm{p}^-.
\end{equation}
Everything on the right hand side is known, so we just need to solve for the coordinates of $\pd{y}\bar q_\sN^G$.
Note that the elements of the vector $\pd{y}\bar{\bm{q}}$ that we solve for in the above expression are the Galerkin coefficients of $\pd{y}\bar q_\sN^G$ in the basis $p_n^q$, not $p_n^\psi$.

Next we need equations for the evolution of $\hat{\vartheta}^\pm$ and $\hat{q}_n$.
The discrete evolution equations for $\hat{\vartheta}^\pm$ are simple because $\pd{z}\bar u_\sN^G=0$ at the boundaries, even if $\pd{z}\bar u \neq0$:
\begin{equation}
(-c+\bar u_\sN^G(z=1))\hat{\vartheta}^+=0,\quad(-c+\bar u_\sN^G(z=0))\hat{\vartheta}^-=0.
\end{equation}
It will be convenient to eliminate $\hat{\psi}_n$ formally as follows
...
Once we have this, we can write the discrete PV evolution equation as follows
\begin{gather}
...
\end{array}\right]\left(\begin{array}{c}\hat{\vartheta}^+\\\hat{q}\\\hat{\vartheta}^-\end{array}\right)
\end{multline}
The top and bottom rows require either $c = \bar u_\sN^G(z=1) = \bar u_\sN^G(z=0)$ or $\hat{\vartheta}^\pm=0$.
The only realistic option is the latter, so the eigenvalue problem simplifies to
\begin{equation}
\left[\bar{\mat{U}} -(\bar{\mat{Q}}_y+\beta\mat{M})\left((k_x^2+k_y^2)\mat{M}+\mat{L}\right)^{-1} \mat{B}\right]\hat{\bm{q}} = c\mat{B}\hat{\bm{q}}.
\end{equation}
\clearpage
The linear instability analysis will in general proceed as follows
\begin{itemize}
\item Choose $\bar{u}$ and $N^2(z)$. (and $f_0$ and $H$ and $\beta$ and $\sN$)
\item Use (31) and (32) to construct the Galerkin approximations $\bar u_\sN^G$ and $\pd{y}\bar q_\sN^G$.
...
If you don't want to use a generalized eigenvalue solver, you could just find eigenvalues of the matrix
\[\mat{B}^{-1}\left[\bar{\mat{U}} -(\bar{\mat{Q}}_y+\beta\mat{M})\left((k_x^2+k_y^2)\mat{M}+\mat{L}\right)^{-1} \mat{B}\right].\]
\end{itemize}
Note that despite what I said in the previous sections about using the diagonalizing basis, this entire section still uses the Shen/Legendre bases.
In particular, the eigenvector will be coefficients of $q_\sN^G$ in the Legendre basis.
